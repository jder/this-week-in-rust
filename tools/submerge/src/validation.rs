    ...
    for event in MarkdownParser::new(text) {
        match event {
            ...
            Event::Start(Tag::Item) => {
                community_items.push(headings.strict.then(CommunityItemState::default));
            }
            ...
            Event::Text(value) => {
                ...
                } else if headings.strict && !in_code_block {
...
fn inspect_html_tags(text: &str, filename: &str, report: &mut Report) {
    ...
    for (event, range) in MarkdownParser::new(text).into_offset_iter() {
        ...
        for captures in HTML_TAG_RE.captures_iter(&value) {
            ...
            if comment_ranges
                .iter()
                .any(|(comment_start, comment_end)| (*comment_start..*comment_end).contains(&start))
            {
                continue;
            }
            if &captures["close"] == "/" {
